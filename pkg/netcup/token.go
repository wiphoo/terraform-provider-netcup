...
// ParseAccessTokenUserID extracts the numeric SCP account id from the JWT access
// token's "id" claim, without verifying its signature. It returns the id as a
// string (used to build /v1/users/{userId}/ssh-keys), mirroring how the legacy
// netcup-api.sh USERID helper derives the account from the token.
//
